...
use crate::json_config::{Config, Market, OpenOrders, ProgramData, TokenAccountData, User};
    ...
    fn from(value: &Config) -> Self {
        Self {
            programs: value
                .programs
                .iter()
                .map(|x| Obv2ProgramData::try_from(x).expect("Program should be Pubkey"))
                .collect_vec(),
            users: value
                .users
                .iter()
                .map(|x| Obv2User::try_from(x).expect("User should be Pubkey"))
                .collect_vec(),
...
impl From<&OpenOrders> for Obv2OpenOrders {
    fn from(value: &OpenOrders) -> Self {
        Self {
            market: convert_to_pk(&value.market),
            open_orders: convert_to_pk(&value.open_orders),
        }
    }
}
...
impl From<&Market> for Obv2Market {
    fn from(value: &Market) -> Self {
        Self {
            ...
            oracle_a: convert_to_pk(&value.oracle_a),
            oracle_b: convert_to_pk(&value.oracle_b),
    ...
    fn from(value: &User) -> Self {
        Self {
            ...
            open_orders: value
                .open_orders
                .iter()
                .map(|x| Obv2OpenOrders::try_from(x).expect("OpenOrders should be Pubkey"))
                .collect_vec(),
            token_data: value
                .token_data
                .iter()
                .map(|x| Obv2TokenAccountData::try_from(x).expect("OpenOrders should be Pubkey"))
                .collect_vec(),
        }
    }
}
